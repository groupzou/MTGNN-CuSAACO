...
from ocpmodels.models.mt_utils import mean_log_Gaussian_like


@registry.register_trainer("mtenergy")
class MTEnergyTrainer(BaseTrainer):
    """
    ...
    def _compute_loss(self, out, batch_list):
        ...
        try:
            drifting_target = torch.cat(
                [(batch.distances_relaxed - batch.distances).to(self.device) for batch in batch_list], dim=0
            )
        except AttributeError:
            drifting_target = []
            for batch in batch_list:
                init_src_node, init_dst_node = batch.edge_index
                relax_src_pos, relax_dst_pos = batch.pos_relaxed[init_src_node], \
                                               batch.pos_relaxed[init_dst_node]
                distances_relaxed = torch.sqrt(F.relu(torch.sum((relax_src_pos - relax_dst_pos) ** 2, -1)))
                drifting_target.append(
                        (distances_relaxed - batch.distances).to(self.device)
                        )
            drifting_target = torch.cat(drifting_target, dim=0)
        ...
        if not self.mdn:
            loss2 = self.loss_fn["energy"](out["drifting"], drifting_target.view([-1, 1]))
        else:
            loss2 = mean_log_Gaussian_like(out["pi"], out["sigma"], out["mu"], drifting_target.view([-1, 1]))
