...
@registry.register_model("cgcnn_dop")
class CGCNNDop(BaseModel):
    ...
    def __init__(
        ...
        super(CGCNNDop, self).__init__(num_atoms, bond_feat_dim, num_targets)
    ...
    def _convolve(self, data):
        ...
        for f in self.convs:
            ...
            dop_feats = node_feats[dop_mask]
            _, batch_counts = torch.unique(data.batch, return_counts=True)
            dop_feat_repeat = dop_feats.repeat_interleave(batch_counts, dim=0)
            node_feats = node_feats + dop_feat_repeat
