...
@registry.register_model("schnet_dop")
class SchNetDopWrap(SchNet):
    ...
    def __init__(
        ...
        super(SchNetDopWrap, self).__init__(
    ...
    def _forward(self, data):
        ...
        if self.use_pbc:
            ...
            for interaction in self.interactions:
                ...
                dop_feats = h[dop_mask]
                _, batch_counts = torch.unique(batch, return_counts=True)
                dop_feat_repeat = dop_feats.repeat_interleave(batch_counts, dim=0)
                h = h + dop_feat_repeat
            ...
            # self.readout == 'add' rather than 'mean' by default, so it is the only way to add dop feats only once
            ...
            energy = super(SchNetDopWrap, self).forward(z, pos, batch)
        return energy
